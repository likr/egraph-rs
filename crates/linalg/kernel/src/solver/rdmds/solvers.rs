...
pub struct CgSolverInstance<S> {
    pub matrix: SparseSymmetricMatrix<S>,
    pub max_iterations: usize,
    pub tolerance: S,
}
...
pub struct IncompleteCholeskyPreconditioner<S> {
    pub n: usize,
    pub diagonal: Vec<S>,
    pub row_entries: Vec<Vec<(usize, S)>>,
    pub col_entries: Vec<Vec<(usize, S)>>,
}
...
pub struct Ic0CgSolverInstance<S> {
    pub matrix: SparseSymmetricMatrix<S>,
    pub preconditioner: IncompleteCholeskyPreconditioner<S>,
    pub max_iterations: usize,
    pub tolerance: S,
}
...
pub struct AmgCgSolverInstance<S> {
    pub matrix: SparseSymmetricMatrix<S>,
    pub amg: crate::solver::rdmds::amg::AmgSolver<S>,
    pub hierarchy: crate::solver::rdmds::amg::hierarchy::Hierarchy<S>,
    pub max_iterations: usize,
    pub tolerance: S,
}
    ...
    S: DrawingValue + Default,
{
    ...
    fn build(&self, matrix: SparseSymmetricMatrix<S>) -> Self::Solver {
        let amg = crate::solver::rdmds::amg::AmgSolver::<S>::default();
        let a_csr = crate::solver::rdmds::amg::matrix::CsrMatrix::from_symmetric(&matrix);
        let hierarchy = crate::solver::rdmds::amg::hierarchy::build_hierarchy(
            a_csr,
            amg.theta,
            amg.max_levels,
            amg.max_coarse_size,
