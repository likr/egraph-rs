...
use crate::solver::rdmds::eigenvalue::{
...
use crate::solver::rdmds::solvers::LinearSolverBuilder;
use crate::Laplacian;
use ndarray::Array2;
    ...
    S: DrawingValue + Default,
{
    ...
    pub fn embedding<G, F, R, Builder>(
        &self,
        graph: G,
        length: F,
        builder: &Builder,
    ...
    pub fn embedding_symmetric_normalized<G, F, R, Builder>(
        &self,
        graph: G,
        length: F,
        builder: &Builder,
    ...
    pub fn embedding_random_walk_normalized<G, F, R, Builder>(
        &self,
        graph: G,
        length: F,
        builder: &Builder,
    ...
    {
        let mut result =
            self.eigendecomposition_random_walk_normalized(graph, length, builder, rng);
        let d = result.eigenvalues.len();
        for dim in 0..d {
            let mut eigenvector = result.eigenvectors.column_mut(dim);
            eigenvector /= result.eigenvalues[dim].max(S::zero()).sqrt();
    ...
    where
        ...
        Builder: LinearSolverBuilder<S>,
    {
        let laplacian = crate::StandardLaplacian
    ...
    where
        ...
        Builder: LinearSolverBuilder<S>,
    {
        let laplacian = crate::SymmetricNormalizedLaplacian
            .build(graph, &mut length)
            .scale_and_shift(S::one(), -self.shift);
