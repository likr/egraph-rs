...
class TestDiGraph(unittest.TestCase):
    def test_constructor(self):
        """
        Test creating a DiGraph instance
        """
        # Create an empty directed graph
        graph = eg.DiGraph()

        # Verify that the instance exists
        self.assertIsInstance(graph, eg.DiGraph)
    ...
    def test_add_remove_edge(self):
        ...
        # Verify edge weights
        self.assertEqual(graph.edge_weight(edge1),
                         "Edge 1->2", "Edge weight should match")
    ...
    def test_contains_find_edge(self):
        ...
        self.assertFalse(graph.contains_edge(node2, node1),
                         "Edge should not exist in reverse direction")
        ...
        with self.assertRaises(Exception):
            graph.find_edge(node2, node1)  # Reverse direction
        with self.assertRaises(Exception):
            graph.find_edge(node1, node3)  # Non-existent edge
    ...
    def test_neighbors(self):
        ...
        graph.add_edge(nodes[4], nodes[2], "4->2")

        # Test neighbors (should return outgoing neighbors for directed graph)
        ...
        self.assertIn(nodes[2], neighbors_1,
                      "Node 2 should be an outgoing neighbor of Node 1")
        self.assertIn(nodes[3], neighbors_1,
                      "Node 3 should be an outgoing neighbor of Node 1")

        # Test neighbors_directed with outgoing direction
        neighbors_directed_0_out = graph.neighbors_directed(
            nodes[0], 0)  # Outgoing
        self.assertEqual(len(neighbors_directed_0_out), 2,
                         "Node 0 should have 2 outgoing neighbors")
        self.assertIn(nodes[1], neighbors_directed_0_out,
                      "Node 1 should be an outgoing neighbor of Node 0")
        self.assertIn(nodes[2], neighbors_directed_0_out,
                      "Node 2 should be an outgoing neighbor of Node 0")

        # Test neighbors_directed with incoming direction
        neighbors_directed_2_in = graph.neighbors_directed(
            nodes[2], 1)  # Incoming
        self.assertEqual(len(neighbors_directed_2_in), 3,
                         "Node 2 should have 3 incoming neighbors")
        self.assertIn(nodes[0], neighbors_directed_2_in,
                      "Node 0 should be an incoming neighbor of Node 2")
        self.assertIn(nodes[1], neighbors_directed_2_in,
                      "Node 1 should be an incoming neighbor of Node 2")
        self.assertIn(nodes[4], neighbors_directed_2_in,
                      "Node 4 should be an incoming neighbor of Node 2")

        # Test neighbors_undirected (should include both incoming and outgoing)
        neighbors_undirected_1 = graph.neighbors_undirected(nodes[1])
        self.assertEqual(len(neighbors_undirected_1), 3,
                         "Node 1 should have 3 undirected neighbors")
        self.assertIn(nodes[0], neighbors_undirected_1,
                      "Node 0 should be an undirected neighbor of Node 1")
        self.assertIn(nodes[2], neighbors_undirected_1,
                      "Node 2 should be an undirected neighbor of Node 1")
        self.assertIn(nodes[3], neighbors_undirected_1,
                      "Node 3 should be an undirected neighbor of Node 1")
    ...
    def test_edges(self):
        ...
        graph.add_edge(node2, node1, "Edge 2->1")  # Reverse edge
        ...
        self.assertIn("Edge 2->1", edges_2,
                      "Edge 2->1 should be connected to Node 2")
    ...
    def test_externals(self):
        ...
        # Test externals with outgoing direction
        externals_out = graph.externals(0)  # Outgoing
        self.assertEqual(len(externals_out), 3,
                         "There should be 3 nodes with no outgoing edges")
        self.assertIn(nodes[2], externals_out,
                      "Node 2 should have no outgoing edges")
        self.assertIn(nodes[3], externals_out,
                      "Node 3 should have no outgoing edges")
        self.assertIn(nodes[4], externals_out,
                      "Node 4 should have no outgoing edges")

        # Test externals with incoming direction
        externals_in = graph.externals(1)  # Incoming
        self.assertEqual(len(externals_in), 2,
                         "There should be 2 nodes with no incoming edges")
        self.assertIn(nodes[0], externals_in,
                      "Node 0 should have no incoming edges")
    ...
    def test_map(self):
        ...
        # The mapped graph is actually a GraphAdapter, not a DiGraph
        # This is expected behavior in the Python bindings
        self.assertIsInstance(mapped_graph, object,
                              "Mapped graph should be a valid object")
    ...
    def test_filter_map(self):
        ...
        # The filtered graph is actually a GraphAdapter, not a DiGraph
        # This is expected behavior in the Python bindings
        self.assertIsInstance(filtered_graph, object,
                              "Filtered graph should be a valid object")

    def test_with_networkx_conversion(self):
        """
        Test conversion from NetworkX directed graph
        """
        ...
        nx_graph.add_edge(3, 1, weight=3.5)  # Create a cycle

        # Convert to egraph DiGraph
        graph = eg.DiGraph()
        indices = {}
        ...
        self.assertEqual(graph.edge_count(), 3,
                         "Converted graph should have 3 edges")
        ...
        self.assertTrue(graph.contains_edge(
            indices[3], indices[1]), "Edge should exist")
        self.assertFalse(graph.contains_edge(
            indices[1], indices[3]), "Edge should not exist")
        self.assertFalse(graph.contains_edge(
            indices[2], indices[1]), "Edge should not exist")
        self.assertFalse(graph.contains_edge(
            indices[3], indices[2]), "Edge should not exist")

    def test_large_graph(self):
        """
        Test with a larger graph (Les Miserables as directed)
        """
        # Create Les Miserables graph as directed
        les_mis_graph = nx.les_miserables_graph()
        les_mis_digraph = nx.DiGraph(les_mis_graph)  # Convert to directed

        graph = eg.DiGraph()
        indices = {}
        for u in les_mis_digraph.nodes:
            indices[u] = graph.add_node(u)
        for u, v in les_mis_digraph.edges:
            graph.add_edge(indices[u], indices[v], (u, v))

        # Verify graph properties
        self.assertEqual(graph.node_count(), len(les_mis_digraph.nodes),
                         "Graph should have the same number of nodes as the NetworkX graph")
        self.assertEqual(graph.edge_count(), len(les_mis_digraph.edges),
        ...
        for u in les_mis_digraph.nodes:
            nx_successors = set(les_mis_digraph.successors(u))
            eg_successors = set(graph.node_weight(n)
                                for n in graph.neighbors(indices[u]))
            self.assertEqual(eg_successors, nx_successors,
                             f"Node {u} should have the same outgoing neighbors in both graphs")

        # Test incoming neighbors
        for u in les_mis_digraph.nodes:
            nx_predecessors = set(les_mis_digraph.predecessors(u))
            eg_predecessors = set(graph.node_weight(n)
                                  for n in graph.neighbors_directed(indices[u], 1))
            self.assertEqual(eg_predecessors, nx_predecessors,
                             f"Node {u} should have the same incoming neighbors in both graphs")
