...
pub fn angular_resolution<G>(graph: G, drawing: &Drawing2D<G::NodeId, f32>) -> f32
where
    ...
    G::NodeId: DrawingIndex,
{
    ...
    for u in graph.node_identifiers() {
        let Tuple2D(x0, y0) = drawing.position(u).unwrap();
        let neighbors = graph.neighbors(u).collect::<Vec<_>>();
        let n = neighbors.len();
        for i in 1..n {
            let v = neighbors[i];
            let Tuple2D(x1, y1) = drawing.position(v).unwrap();
            for j in 0..i {
                let w = neighbors[j];
                let Tuple2D(x2, y2) = drawing.position(w).unwrap();
