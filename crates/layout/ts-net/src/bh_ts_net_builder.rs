...
/// Builder for constructing a `BhTsNet` layout instance with customized hyperparameters.
#[derive(Debug, Clone)]
pub struct BhTsNetBuilder<S> {
    pub perplexity: S,
    pub theta: S,
    pub k: Option<usize>,
...
impl<S> BhTsNetBuilder<S>
where
    S: DrawingValue + Float + std::iter::Sum + std::ops::AddAssign + Default,
{
    /// Creates a new `BhTsNetBuilder` initialized with default hyperparameters.
    pub fn new() -> Self {
        Self {
    ...
    /// Sets the target perplexity (effective number of neighbors, default: 40.0).
    ...
    /// Sets the number of nearest neighbors `k` for Partial BFS. If `None`, defaults to `3 * perplexity`.
    ...
    /// Validates the hyperparameters and builds a new `BhTsNet` instance.
    pub fn build(self) -> Result<BhTsNet<S>, String> {
...
impl<S> Default for BhTsNetBuilder<S>
where
    S: DrawingValue + Float + std::iter::Sum + std::ops::AddAssign + Default,
{
    fn default() -> Self {
