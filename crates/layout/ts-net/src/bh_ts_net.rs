use crate::{
    bh_ts_net_builder::BhTsNetBuilder, partial_bfs::compute_partial_bfs_probabilities,
    quadtree::Quadtree,
};
...
/// BH-tsNET (Barnes-Hut tsNET) graph layout algorithm.
///
/// Reduces the computational complexity of tsNET from O(N^3) or O(NM) to O(N log N) by combining:
/// - (C0) O(N) Partial Breadth-First Search for high-dimensional Gaussian probabilities.
/// - (C1) O(N log N) Barnes-Hut Quadtree approximation for low-dimensional Student-t KL repulsion.
/// - (C2) O(N log N) Barnes-Hut Quadtree approximation for entropy / repulsion gradient.
#[derive(Debug, Clone)]
pub struct BhTsNet<S> {
    pub perplexity: S,
    pub theta: S,
    pub k: usize,
...
{
    /// Creates a builder to configure and construct a `BhTsNet` layout instance.
    pub fn builder() -> BhTsNetBuilder<S> {
        BhTsNetBuilder::new()
    }

    /// Creates a `BhTsNet` instance with default hyperparameters.
    pub fn new() -> Self {
        BhTsNetBuilder::new().build().unwrap()
    }

    /// Executes the BH-tsNET layout algorithm on the graph and drawing.
        ...
        R: Rng + ?Sized,
    {
        ...
        for &(iters, exaggeration, lambda_kl, lambda_c, lambda_r) in &stages {
            ...
            for _ in 0..iters {
                // Build 2D Quadtree from current coordinates
                let quadtree = Quadtree::build(&points);

                // Phase A: Compute exact sparse attraction forces and Barnes-Hut approximations
                let mut attr_forces = vec![[S::zero(), S::zero()]; n];
                let mut rep_forces_raw = vec![[S::zero(), S::zero()]; n];
                let mut ent_forces_raw = vec![[S::zero(), S::zero()]; n];
                let mut z_sums = vec![S::zero(); n];

                for i in 0..n {
                    let xi = points[i][0];
                    let yi = points[i][1];

                    // Exact sparse attraction forces: sum_{(j, p_ij)} p_ij * (1 + ||y_i - y_j||^2)^(-1) * (y_i - y_j)
                    ...
                    // Barnes-Hut KL Repulsion traversal
                    let mut z_i = S::zero();
                    let mut f_rep_x = S::zero();
                    let mut f_rep_y = S::zero();
                    quadtree.root.compute_kl_repulsion(
                        i,
                        xi,
                        yi,
                        theta_sq,
                    ...
                    z_sums[i] = z_i;
                    rep_forces_raw[i] = [f_rep_x, f_rep_y];

                    // Barnes-Hut Entropy / Repulsion traversal
                    if lambda_r > S::zero() {
                ...
                // Global normalization constant Z = sum_i z_sums[i]
                ...
                for i in 0..n {
                    ...
                    // KL Repulsion force
                    let f_rep_x = rep_forces_raw[i][0] / total_z;
                    let f_rep_y = rep_forces_raw[i][1] / total_z;

                    // KL Divergence gradient = 4 * (F_attr - F_rep)
                    let grad_kl_x = four_s * (attr_forces[i][0] - f_rep_x);
                    let grad_kl_y = four_s * (attr_forces[i][1] - f_rep_y);
