...
/// Builder for constructing an `LTsNet` (Linear-tsNET) layout instance with customized hyperparameters.
#[derive(Debug, Clone)]
pub struct LTsNetBuilder<S> {
    pub intervals: usize,
...
impl<S> LTsNetBuilder<S>
where
    S: DrawingValue + Float + std::iter::Sum + std::ops::AddAssign + Default,
{
    /// Creates a new `LTsNetBuilder` initialized with default hyperparameters.
    pub fn new() -> Self {
        Self {
            intervals: 25,
    ...
    /// Sets the repulsion offset epsilon_r (default: 0.05).
    ...
    /// Validates the hyperparameters and builds a new `LTsNet` instance.
    pub fn build(self) -> Result<LTsNet<S>, String> {
        if self.intervals == 0 {
...
impl<S> Default for LTsNetBuilder<S>
where
    S: DrawingValue + Float + std::iter::Sum + std::ops::AddAssign + Default,
{
    fn default() -> Self {
