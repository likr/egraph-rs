...
fn test_metric_spherical_2d_grid() {
    ...
    for i in 0..lon_steps {
        ...
        for j in 0..lat_steps {
            let lat = j as f32 * std::f32::consts::PI / (lat_steps as f32)
                - std::f32::consts::PI / 2.0f32;
            ...
            for k in 0..lon_steps {
                ...
                for l in 0..lat_steps {
                    let other_lat = l as f32 * std::f32::consts::PI / (lat_steps as f32)
                        - std::f32::consts::PI / 2.0f32;
...
fn test_metric_spherical_2d_sgd_simulation() {
    ...
    if norm > 0.0f32 {
        ...
        assert!(
            !is_nan(p1_moved.0),
            "Result longitude contains NaN after SGD-like operation"
        );
        assert!(
            !is_nan(p1_moved.1),
            "Result latitude contains NaN after SGD-like operation"
        );
