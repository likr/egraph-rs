...
use egraph_layout_force_directed::initial_placement;
use egraph_layout_force_directed::simulation::Simulation;
use egraph_layout_force_directed::Point;
use rand::prelude::*;
...
fn edge_length<N, E, Ty: EdgeType, Ix: IndexType>(
    graph: &Graph,
    link_distance_accessor: &Box<Fn(&Graph, EdgeIndex) -> f32>,
    a: NodeIndex,
    b: NodeIndex,
) -> f32 {
...
fn collapse<N, E, Ty: EdgeType, Ix: IndexType>(
    ...
    node_parents: &Vec<usize>,
    node_types: &Vec<NodeType>,
    shrink_node: &Box<Fn(&Graph, &Vec<NodeIndex>) -> N>,
    shrink_edge: &Box<Fn(&Graph, &Vec<EdgeIndex>, f32) -> E>,
    link_distance_accessor: &Box<Fn(&Graph, EdgeIndex) -> f32>,
) -> Graph {
...
pub struct FM3<N, E, Ty: EdgeType, Ix: IndexType> {
    ...
    pub shrink_node: Box<Fn(&Graph, &Vec<NodeIndex>) -> N>,
    pub shrink_edge: Box<Fn(&Graph, &Vec<EdgeIndex>, f32) -> E>,
    pub link_distance_accessor: Box<Fn(&Graph, EdgeIndex) -> f32>,
}

impl<N, E, Ty: EdgeType, Ix: IndexType> FM3 {
    pub fn new(
        shrink_node: Box<Fn(&Graph, &Vec<NodeIndex>) -> N>,
        shrink_edge: Box<Fn(&Graph, &Vec<EdgeIndex>, f32) -> E>,
        link_distance_accessor: Box<Fn(&Graph, EdgeIndex) -> f32>,
    ...
    pub fn call(&self, graph: &Graph, simulation: &Simulation) -> Vec<Point> {
        let seed = [0; 32];
