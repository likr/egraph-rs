...
pub struct LinkForce {
    pub strength: Box<Fn(&Graph, NodeIndex, NodeIndex) -> f32>,
...
impl LinkForce {
    pub fn new() -> LinkForce {
        LinkForce {
            strength: Box::new(|graph, source, target| {
                let source_degree = degree(graph, source);
                let target_degree = degree(graph, target);
...
impl Force for LinkForce {
    fn build(&self, graph: &Graph) -> Box<ForceContext> {
        ...
        let links = graph
            .edges()
            .map(|(source, target)| {
                let distance = distance_accessor(graph, source, target);
                let strength = strength_accessor(graph, source, target);
                let source_degree = degree(graph, source) as f32;
                let target_degree = degree(graph, target) as f32;
                let bias = source_degree / (source_degree + target_degree);
                Link::new(source, target, distance, strength, bias)
            })
            .collect();
